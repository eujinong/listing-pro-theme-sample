		...
		if(!empty($existingCoupons)){
			foreach($existingCoupons as $singleCoupon){
				...
				if($singleCOde==$code){
					...
					$cstarts = date("m-d-Y", strtotime($cstarts));
					$cends = $singleCoupon['ends'];
					$cends = date("m-d-Y", strtotime($cends));
	...
	function listingpro_coupons_page(){
		...
		<div class="wrap listingpro-coupons">
				...
				<div class="clearfix">
						<form name="lp_add_coupon_form" method="POST" action="">
							...
							<div class="form-group col-md-6">
								<label for="couponpercentage"><?php echo $discountLabel; ?></label>
			...
			<ul class="subsubsub lpbackendtabs">
				<li class="all tab-link current" data-tab="tab-1"><a class="current">All <span class="count">(<?php echo $allCount; ?>)</span></a> |</li>
